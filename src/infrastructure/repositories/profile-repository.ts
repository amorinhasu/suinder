import type { LookingForOption, ProfileInput, ProfileStatus, UserProfile } from '../../domain/profile.js';
...
export class ProfileRepository {
  ...
  public async update(guildId: string, profileId: string, input: Omit<ProfileInput, 'guildId' | 'discordUserId' | 'avatarUrl'>): Promise<UserProfile> {
    const result = await this.database.query<ProfileRow>(
      `
        ...
        returning *
      `,
      [profileId, input.displayName, input.age, input.bio, input.lookingFor, input.receiveDm, input.consentedAt, guildId]
    );
  ...
  public async findNextDiscoverableProfile(guildId: string, viewerProfileId: string): Promise<UserProfile | null> {
    const result = await this.database.query<ProfileRow>(
      `
        select target.*
        ...
        limit 1
      `,
      [guildId, viewerProfileId]
    );

    return result.rows[0] ? mapProfileRow(result.rows[0]) : null;
  }

  public async findDiscoverableCandidates(guildId: string, viewerProfileId: string, limit = 20): Promise<UserProfile[]> {
    const result = await this.database.query<ProfileRow>(
      `
        select target.*
        from user_profiles target
  ...
  public async recordLikeAndMaybeCreateMatch(guildId: string, actorProfileId: string, targetProfileId: string): Promise<LikeResult> {
